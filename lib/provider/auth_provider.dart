...
class AuthProvider with ChangeNotifier {
  ...
  Future<ResponseModel> registration(SignUpModel signUpModel, context) async {
    ...
    if(map['coupon']!=null){
      ...
      Provider.of<CouponProvider>(context, listen: false)
          .giftCoupon(CouponModel.fromJson(map['coupon']));
    }
      setSignUp();
  ...
  Future<ResponseModel> forgetPassword(String email) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      responseModel =
          ResponseModel(true, jsonDecode(apiResponse.response.body)['message']);
  ...
  Future<ResponseModel> checkEmail(String email) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      responseModel =
          ResponseModel(true, jsonDecode(apiResponse.response.body)["token"]);
    } else {
      String errorMessage;
      if (apiResponse.error is String) {
        print(apiResponse.error.toString());
  ...
  Future<ResponseModel> verifyPhone(String phone) async {
    ...
    notifyListeners();
    ApiResponse apiResponse =
        await authRepo.verifyPhone(_phoneNumber, _verificationCode);
